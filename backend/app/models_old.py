...
class SANSAResponse(Base):
    ...
    item_score = Column(DECIMAL(10, 2))

    # Relationships
    sansa_response = relationship("SANSAResponse", back_populates="items")

    __table_args__ = (
        UniqueConstraint('sansa_response_id', 'item_code', name='unique_response_item'),
    )
...
class SatisfactionResponse(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    satisfaction_response_id = Column(Integer, ForeignKey("satisfaction_responses.id", ondelete="CASCADE"), nullable=False, index=True)
    item_number = Column(Integer, nullable=False)
    ...
    # Relationships
    satisfaction_response = relationship("SatisfactionResponse", back_populates="items")

    __table_args__ = (
        UniqueConstraint('satisfaction_response_id', 'item_code', name='unique_response_item'),
    )
...
class MNAResponse(Base):
    ...
    # Relationships
    mna_response = relationship("MNAResponse", back_populates="items")

    __table_args__ = (
        UniqueConstraint('mna_response_id', 'item_code', name='unique_response_item'),
    )
